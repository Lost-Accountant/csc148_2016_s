...
def is_full1(t, n):
    ...
    if len(t.children) == 0:
        return True
    # base case 2: has children but not full children
    elif len(t.children) < n:
        return False

    # general case: recursion on each child
    else:
        return all([is_full1(child, n) for child in t.children])
...
def is_full2(t, n):
    ...
    # general case but no recursion
    else:
        i = 0
        # goes through each child
        while i < len(t.children) and is_full2(t.children[i], n):
            i += 1
            # count how many children satisfied
        # when out of loop, either gone through all kids or one not full
        return i == len(t.children)
...
def is_full3(t, n):
    ...
    while queue.is_empty() is False:
        ...
        if len(extract.children) == 0:
            pass
        elif len(extract.children) != n:
            return False
        else:
            # when has n children
            for kid in extract.children:
                queue.add(kid)
